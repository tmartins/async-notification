...
public class AdvancedNotificationEventListener implements
        PostCommitFilteringEventListener {

    private static final String USERPROFILE_NOTIFICATIONS_PROPERTY = "userprofile:notifications";

    public static final String IMMEDIATE_SUBSCRIPTION = "immediate";

    private static final Log log = LogFactory.getLog(AdvancedNotificationEventListener.class);
    ...
    private Map<String, String> userSubscriptionPrefs = new HashMap<String, String>();
    ...
    public boolean acceptEvent(Event event) {
        ...
        return notificationService.getNotificationEventNames().contains(
                event.getName());
    }
    ...
    private void sendNotificationSignalForUser(Notification notification,
        ...
        userUrl.append(notificationService.getServerUrlPrefix()).append("user/").append(
        ...
        if (bh != null && bh.getBlob() != null) {
            ...
            docMainFile.append(notificationService.getServerUrlPrefix()).append(
        ...
        if (isInterestedInNotification(notification)) {
            if (hasSubscribedToInstantNotification(
                    (String) eventInfo.get(NotificationConstants.DESTINATION_KEY),
                    event)) {
                try {
                ...
                addActivity(ctx, event, subscriptor);
            }
        }
    }
    ...
    private void gatherConcernedUsersForDocument(CoreSession coreSession,
        ...
        if (doc.getPath().segmentCount() > 1) {
            ...
            if (doc.getParentRef() != null
                    && coreSession.exists(doc.getParentRef())) {
    ...
    protected void addActivity(DocumentEventContext docEventContext,
            Event event, String targetUser) {
        Activity activity = toActivity(docEventContext, event, targetUser);
        ActivityStreamService activityStreamService = Framework.getLocalService(ActivityStreamService.class);
        activityStreamService.addActivity(activity);
    }

    /**
     * Hack the current activity object 
     * - actor/displayActor = user responsible for the notfication 
     * - context = document ref 
     * - object = kind of subscription (daily/weekly) 
     * - target/displayTarget = user to who the notification will be sent 
     * - verb = event that corresponds to the notification
     */
    private Activity toActivity(DocumentEventContext docEventContext,
        ...
        return new ActivityBuilder().actor(
                ...
                event.getName()).object(userSubscriptionPrefs.get(targetUser)).displayObject(
                ActivityHelper.getDocumentTitle(doc)).target(targetUser).displayTarget(targetUser).context(
                ActivityHelper.createDocumentActivityObject(doc)).build();
    }

    protected boolean hasSubscribedToInstantNotification(String username,
            Event event) {
        try {
            if (!userSubscriptionPrefs.containsKey(username)) {
                DocumentModel userProfile = getUserProfileService().getUserProfileDocument(
                        username, event.getContext().getCoreSession());
                if (userProfile != null) {
                    userSubscriptionPrefs.put(
                            username,
                            (String) userProfile.getPropertyValue(USERPROFILE_NOTIFICATIONS_PROPERTY));
                } else {
                    // default behavior
                    userSubscriptionPrefs.put(username, IMMEDIATE_SUBSCRIPTION);
                }
            }
            if (IMMEDIATE_SUBSCRIPTION.equals(userSubscriptionPrefs.get(username))) {
                return true;
            }
            return false;
        } catch (ClientException e) {
